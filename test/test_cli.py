...
class TestAlias(object):
    ...
    def test_defined(self, runner, cli, default_config_file):
    ...
    def test_with_config(self, runner, cli):
        with open('config1', 'wb') as f:
            f.write("[alias]\napps=list apps")
...
class TestLogin(object):
    ...
    def test_no_credentials(self, runner, cli, default_config_file):
    ...
    def test_with_credentials(self, runner, cli, default_config_file):
    ...
    def test_with_config_and_profile(self, runner, cli):
        with open('test.ini', 'wb') as f:
            f.write("[p1]\nendpoint = https://example.com")

        with mock.patch('slipstream.cli.api.Api.verify', return_value=True):
            result = runner.invoke(cli, ['-p', 'p1', '-c', 'test.ini', 'login'],
                                   input=("bob\nstaple horse\n"))
        ...
        assert parser.get('p1', 'username') == 'bob'
        assert parser.get('p1', 'password') == 'staple horse'
...
@pytest.mark.usefixtures('verified_auth')
